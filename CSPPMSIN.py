...
lista_contratistas = ["LUDIN 2026 REV 7", "VENRINOS 2026 REV 6"]
...
with st.sidebar:
    ...
    # 3.1 Filtro Principal Consolidado
...
if depto_seleccionado == "GENERAL":
    ...
    df_filtrado = df_tareas[df_tareas['Departamento'].isin(lista_contratistas)].copy()
...
st.markdown(f"<h2><i class='fas fa-industry' style='color:#1f77b4; margin-right: 10px;'></i>{depto_seleccionado} <span style='font-size: 13px; font-weight: normal; color: gray; margin-left: 10px;'>| {texto_corte}</span></h2>", unsafe_allow_html=True)
...
with col_main:
    ...
    if depto_seleccionado in ["GENERAL", "CONTRATISTAS", "PLANTA"]:
        ...
        # Filtro para departamentos a evaluar en el loop dependiendo del grupo seleccionado
        if depto_seleccionado == "GENERAL":
            deptos_a_comparar = deptos_unicos
        elif depto_seleccionado == "CONTRATISTAS":
            deptos_a_comparar = [d for d in deptos_unicos if d in lista_contratistas]
        elif depto_seleccionado == "PLANTA":
            deptos_a_comparar = [d for d in deptos_unicos if d not in lista_contratistas]
        ...
        # 1. Creación del DataFrame base y separación del Total de los Departamentos
        ...
        if tipo_grafico == "Barras Horizontales":
            # Total como índice 0 para ubicarlo al fondo del gráfico, y los Departamentos en orden ascendente de EV
            ...
            # Total debe ir al final para que la línea de PV y EV se dibuje sobre los Departamentos
